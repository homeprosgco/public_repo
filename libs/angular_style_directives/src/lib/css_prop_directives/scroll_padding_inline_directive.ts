...
@Directive({
  selector: '[sgScrollPaddingInline]',
  standalone: true
})
export class ScrollPaddingInlineDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgScrollPaddingInline = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgScrollPaddingInline() ?? '';
      ...
      this.el.nativeElement.style.scrollPaddingInline = applied ?? '';

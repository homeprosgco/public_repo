...
@Directive({
  selector: '[sgTextRendering]',
  standalone: true
})
export class TextRenderingDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgTextRendering = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgTextRendering() ?? '';
      ...
      this.el.nativeElement.style.textRendering = applied ?? '';

...
@Directive({
  selector: '[sgBorderBottomLeftRadius]',
  standalone: true
})
export class BorderBottomLeftRadiusDirective {
  ...
  sgBorderBottomLeftRadius = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBorderBottomLeftRadius() ?? '';
      ...
      this.el.nativeElement.style.borderBottomLeftRadius = applied ?? '';

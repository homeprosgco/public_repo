...
@Directive({
  selector: '[sgBorderTopLeftRadius]',
  standalone: true
})
export class BorderTopLeftRadiusDirective {
  ...
  sgBorderTopLeftRadius = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBorderTopLeftRadius() ?? '';
      ...
      this.el.nativeElement.style.borderTopLeftRadius = applied ?? '';

...
@Directive({
  selector: '[sgMarkerEnd]',
  standalone: true
})
export class MarkerEndDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgMarkerEnd = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgMarkerEnd() ?? '';
      ...
      this.el.nativeElement.style.markerEnd = applied ?? '';

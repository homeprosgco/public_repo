...
@Directive({
  selector: '[sgAnimationTimingFunction]',
  standalone: true
})
export class AnimationTimingFunctionDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgAnimationTimingFunction = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgAnimationTimingFunction() ?? '';
      ...
      this.el.nativeElement.style.animationTimingFunction = applied ?? '';

...
export class BorderTopColorDirective {
  ...
  sgBorderTopColor = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBorderTopColor() ?? '';
      ...
      this.el.nativeElement.style.borderTopColor = applied ?? '';

...
@Directive({
  selector: '[sgBorderImageRepeat]',
  standalone: true
})
export class BorderImageRepeatDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgBorderImageRepeat = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBorderImageRepeat() ?? '';
      ...
      this.el.nativeElement.style.borderImageRepeat = applied ?? '';

...
@Directive({
  selector: '[sgGridRowStart]',
  standalone: true
})
export class GridRowStartDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgGridRowStart = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgGridRowStart() ?? '';
      ...
      this.el.nativeElement.style.gridRowStart = applied ?? '';

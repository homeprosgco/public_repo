...
@Directive({
  selector: '[sgFontSizeAdjust]',
  standalone: true
})
export class FontSizeAdjustDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgFontSizeAdjust = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgFontSizeAdjust() ?? '';
      ...
      this.el.nativeElement.style.fontSizeAdjust = applied ?? '';

...
@Directive({
  selector: '[sgMarginBlockEnd]',
  standalone: true
})
export class MarginBlockEndDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgMarginBlockEnd = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgMarginBlockEnd() ?? '';
      ...
      this.el.nativeElement.style.marginBlockEnd = applied ?? '';

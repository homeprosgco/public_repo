...
export class BackgroundColorDirective {
  ...
  sgBackgroundColor = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBackgroundColor() ?? '';
      ...
      this.el.nativeElement.style.backgroundColor = applied ?? '';

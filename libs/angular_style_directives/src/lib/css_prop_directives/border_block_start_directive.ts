...
@Directive({
  selector: '[sgBorderBlockStart]',
  standalone: true
})
export class BorderBlockStartDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgBorderBlockStart = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBorderBlockStart() ?? '';
      ...
      this.el.nativeElement.style.borderBlockStart = applied ?? '';

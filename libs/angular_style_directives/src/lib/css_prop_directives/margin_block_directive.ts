...
@Directive({
  selector: '[sgMarginBlock]',
  standalone: true
})
export class MarginBlockDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgMarginBlock = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgMarginBlock() ?? '';
      ...
      this.el.nativeElement.style.marginBlock = applied ?? '';

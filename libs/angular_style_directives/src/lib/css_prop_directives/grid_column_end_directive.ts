...
@Directive({
  selector: '[sgGridColumnEnd]',
  standalone: true
})
export class GridColumnEndDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgGridColumnEnd = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgGridColumnEnd() ?? '';
      ...
      this.el.nativeElement.style.gridColumnEnd = applied ?? '';

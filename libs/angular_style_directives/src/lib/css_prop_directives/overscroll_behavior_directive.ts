...
@Directive({
  selector: '[sgOverscrollBehavior]',
  standalone: true
})
export class OverscrollBehaviorDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgOverscrollBehavior = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgOverscrollBehavior() ?? '';
      ...
      this.el.nativeElement.style.overscrollBehavior = applied ?? '';

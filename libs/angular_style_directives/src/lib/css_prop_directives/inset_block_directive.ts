...
@Directive({
  selector: '[sgInsetBlock]',
  standalone: true
})
export class InsetBlockDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgInsetBlock = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgInsetBlock() ?? '';
      ...
      this.el.nativeElement.style.insetBlock = applied ?? '';

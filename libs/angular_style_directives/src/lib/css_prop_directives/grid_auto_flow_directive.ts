...
@Directive({
  selector: '[sgGridAutoFlow]',
  standalone: true
})
export class GridAutoFlowDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgGridAutoFlow = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgGridAutoFlow() ?? '';
      ...
      this.el.nativeElement.style.gridAutoFlow = applied ?? '';

...
@Directive({
  selector: '[sgFontStretch]',
  standalone: true
})
export class FontStretchDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgFontStretch = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgFontStretch() ?? '';
      ...
      this.el.nativeElement.style.fontStretch = applied ?? '';

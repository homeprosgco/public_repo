...
@Directive({
  selector: '[sgTextEmphasisPosition]',
  standalone: true
})
export class TextEmphasisPositionDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgTextEmphasisPosition = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgTextEmphasisPosition() ?? '';
      ...
      this.el.nativeElement.style.textEmphasisPosition = applied ?? '';

...
export class BackgroundRepeatDirective {
  ...
  sgBackgroundRepeat = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBackgroundRepeat() ?? '';
      ...
      this.el.nativeElement.style.backgroundRepeat = applied ?? '';

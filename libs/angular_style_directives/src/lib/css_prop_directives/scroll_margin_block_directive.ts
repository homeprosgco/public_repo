...
@Directive({
  selector: '[sgScrollMarginBlock]',
  standalone: true
})
export class ScrollMarginBlockDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgScrollMarginBlock = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgScrollMarginBlock() ?? '';
      ...
      this.el.nativeElement.style.scrollMarginBlock = applied ?? '';

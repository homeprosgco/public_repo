...
@Directive({
  selector: '[sgScrollMarginRight]',
  standalone: true
})
export class ScrollMarginRightDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgScrollMarginRight = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgScrollMarginRight() ?? '';
      ...
      this.el.nativeElement.style.scrollMarginRight = applied ?? '';

...
@Directive({
  selector: '[sgListStyleType]',
  standalone: true
})
export class ListStyleTypeDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgListStyleType = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgListStyleType() ?? '';
      ...
      this.el.nativeElement.style.listStyleType = applied ?? '';

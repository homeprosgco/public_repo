...
@Directive({
  selector: '[sgBorderBlockEnd]',
  standalone: true
})
export class BorderBlockEndDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgBorderBlockEnd = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBorderBlockEnd() ?? '';
      ...
      this.el.nativeElement.style.borderBlockEnd = applied ?? '';

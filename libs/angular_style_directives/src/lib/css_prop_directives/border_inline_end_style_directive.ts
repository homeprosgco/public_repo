...
@Directive({
  selector: '[sgBorderInlineEndStyle]',
  standalone: true
})
export class BorderInlineEndStyleDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgBorderInlineEndStyle = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBorderInlineEndStyle() ?? '';
      ...
      this.el.nativeElement.style.borderInlineEndStyle = applied ?? '';

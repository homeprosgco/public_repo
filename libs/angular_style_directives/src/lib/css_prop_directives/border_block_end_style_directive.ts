...
@Directive({
  selector: '[sgBorderBlockEndStyle]',
  standalone: true
})
export class BorderBlockEndStyleDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgBorderBlockEndStyle = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBorderBlockEndStyle() ?? '';
      ...
      this.el.nativeElement.style.borderBlockEndStyle = applied ?? '';

...
@Directive({
  selector: '[sgPlaceItems]',
  standalone: true
})
export class PlaceItemsDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgPlaceItems = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgPlaceItems() ?? '';
      ...
      this.el.nativeElement.style.placeItems = applied ?? '';

...
@Directive({
  selector: '[sgMinInlineSize]',
  standalone: true
})
export class MinInlineSizeDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgMinInlineSize = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgMinInlineSize() ?? '';
      ...
      this.el.nativeElement.style.minInlineSize = applied ?? '';

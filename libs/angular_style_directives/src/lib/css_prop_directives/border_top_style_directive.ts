...
@Directive({
  selector: '[sgBorderTopStyle]',
  standalone: true
})
export class BorderTopStyleDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgBorderTopStyle = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBorderTopStyle() ?? '';
      ...
      this.el.nativeElement.style.borderTopStyle = applied ?? '';

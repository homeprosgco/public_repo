...
@Directive({
  selector: '[sgGridAutoRows]',
  standalone: true
})
export class GridAutoRowsDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgGridAutoRows = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgGridAutoRows() ?? '';
      ...
      this.el.nativeElement.style.gridAutoRows = applied ?? '';

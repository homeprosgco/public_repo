...
@Directive({
  selector: '[sgBorderImageOutset]',
  standalone: true
})
export class BorderImageOutsetDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgBorderImageOutset = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBorderImageOutset() ?? '';
      ...
      this.el.nativeElement.style.borderImageOutset = applied ?? '';

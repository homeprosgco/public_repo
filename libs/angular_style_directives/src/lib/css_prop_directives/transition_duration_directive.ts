...
@Directive({
  selector: '[sgTransitionDuration]',
  standalone: true
})
export class TransitionDurationDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgTransitionDuration = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgTransitionDuration() ?? '';
      ...
      this.el.nativeElement.style.transitionDuration = applied ?? '';

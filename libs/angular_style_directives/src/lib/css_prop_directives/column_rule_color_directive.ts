...
@Directive({
  selector: '[sgColumnRuleColor]',
  standalone: true
})
export class ColumnRuleColorDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgColumnRuleColor = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgColumnRuleColor() ?? '';
      ...
      this.el.nativeElement.style.columnRuleColor = applied ?? '';

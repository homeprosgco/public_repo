...
@Directive({
  selector: '[sgAlignItems]',
  standalone: true
})
export class AlignItemsDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgAlignItems = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgAlignItems() ?? '';
      ...
      this.el.nativeElement.style.alignItems = applied ?? '';

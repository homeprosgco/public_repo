...
@Directive({
  selector: '[sgOutlineWidth]',
  standalone: true
})
export class OutlineWidthDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgOutlineWidth = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgOutlineWidth() ?? '';
      ...
      this.el.nativeElement.style.outlineWidth = applied ?? '';

...
@Directive({
  selector: '[sgBorderBlockStartStyle]',
  standalone: true
})
export class BorderBlockStartStyleDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgBorderBlockStartStyle = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBorderBlockStartStyle() ?? '';
      ...
      this.el.nativeElement.style.borderBlockStartStyle = applied ?? '';

...
@Directive({
  selector: '[sgAlignSelf]',
  standalone: true
})
export class AlignSelfDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgAlignSelf = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgAlignSelf() ?? '';
      ...
      this.el.nativeElement.style.alignSelf = applied ?? '';

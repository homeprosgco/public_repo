...
@Directive({
  selector: '[sgBorderEndStartRadius]',
  standalone: true
})
export class BorderEndStartRadiusDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgBorderEndStartRadius = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBorderEndStartRadius() ?? '';
      ...
      this.el.nativeElement.style.borderEndStartRadius = applied ?? '';

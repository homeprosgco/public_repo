...
@Directive({
  selector: '[sgObjectPosition]',
  standalone: true
})
export class ObjectPositionDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgObjectPosition = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgObjectPosition() ?? '';
      ...
      this.el.nativeElement.style.objectPosition = applied ?? '';

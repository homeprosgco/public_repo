...
@Directive({
  selector: '[sgMaskPosition]',
  standalone: true
})
export class MaskPositionDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgMaskPosition = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgMaskPosition() ?? '';
      ...
      this.el.nativeElement.style.maskPosition = applied ?? '';

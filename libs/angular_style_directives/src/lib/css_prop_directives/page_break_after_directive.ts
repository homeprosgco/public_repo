...
@Directive({
  selector: '[sgPageBreakAfter]',
  standalone: true
})
export class PageBreakAfterDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgPageBreakAfter = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgPageBreakAfter() ?? '';
      ...
      this.el.nativeElement.style.pageBreakAfter = applied ?? '';

...
@Directive({
  selector: '[sgOffsetPosition]',
  standalone: true
})
export class OffsetPositionDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgOffsetPosition = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgOffsetPosition() ?? '';
      ...
      this.el.nativeElement.style.offsetPosition = applied ?? '';

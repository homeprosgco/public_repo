...
export class BackgroundPositionDirective {
  ...
  sgBackgroundPosition = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBackgroundPosition() ?? '';
      ...
      this.el.nativeElement.style.backgroundPosition = applied ?? '';

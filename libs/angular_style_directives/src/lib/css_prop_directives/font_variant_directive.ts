...
@Directive({
  selector: '[sgFontVariant]',
  standalone: true
})
export class FontVariantDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgFontVariant = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgFontVariant() ?? '';
      ...
      this.el.nativeElement.style.fontVariant = applied ?? '';

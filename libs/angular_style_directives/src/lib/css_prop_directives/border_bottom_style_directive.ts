...
@Directive({
  selector: '[sgBorderBottomStyle]',
  standalone: true
})
export class BorderBottomStyleDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgBorderBottomStyle = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBorderBottomStyle() ?? '';
      ...
      this.el.nativeElement.style.borderBottomStyle = applied ?? '';

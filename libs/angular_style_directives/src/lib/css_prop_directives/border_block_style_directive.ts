...
@Directive({
  selector: '[sgBorderBlockStyle]',
  standalone: true
})
export class BorderBlockStyleDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgBorderBlockStyle = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBorderBlockStyle() ?? '';
      ...
      this.el.nativeElement.style.borderBlockStyle = applied ?? '';

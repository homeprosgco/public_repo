...
@Directive({
  selector: '[sgBorderBlockEndWidth]',
  standalone: true
})
export class BorderBlockEndWidthDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgBorderBlockEndWidth = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBorderBlockEndWidth() ?? '';
      ...
      this.el.nativeElement.style.borderBlockEndWidth = applied ?? '';

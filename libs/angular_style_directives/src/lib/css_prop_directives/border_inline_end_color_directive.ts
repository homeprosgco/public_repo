...
@Directive({
  selector: '[sgBorderInlineEndColor]',
  standalone: true
})
export class BorderInlineEndColorDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgBorderInlineEndColor = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBorderInlineEndColor() ?? '';
      ...
      this.el.nativeElement.style.borderInlineEndColor = applied ?? '';

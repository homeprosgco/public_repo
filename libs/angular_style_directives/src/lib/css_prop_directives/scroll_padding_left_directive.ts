...
@Directive({
  selector: '[sgScrollPaddingLeft]',
  standalone: true
})
export class ScrollPaddingLeftDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgScrollPaddingLeft = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgScrollPaddingLeft() ?? '';
      ...
      this.el.nativeElement.style.scrollPaddingLeft = applied ?? '';

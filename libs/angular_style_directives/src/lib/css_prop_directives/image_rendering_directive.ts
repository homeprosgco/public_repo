...
@Directive({
  selector: '[sgImageRendering]',
  standalone: true
})
export class ImageRenderingDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgImageRendering = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgImageRendering() ?? '';
      ...
      this.el.nativeElement.style.imageRendering = applied ?? '';

...
@Directive({
  selector: '[sgMinBlockSize]',
  standalone: true
})
export class MinBlockSizeDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgMinBlockSize = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgMinBlockSize() ?? '';
      ...
      this.el.nativeElement.style.minBlockSize = applied ?? '';

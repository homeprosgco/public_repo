...
@Directive({
  selector: '[sgGridAutoColumns]',
  standalone: true
})
export class GridAutoColumnsDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgGridAutoColumns = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgGridAutoColumns() ?? '';
      ...
      this.el.nativeElement.style.gridAutoColumns = applied ?? '';

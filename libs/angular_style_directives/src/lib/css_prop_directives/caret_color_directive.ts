...
@Directive({
  selector: '[sgCaretColor]',
  standalone: true
})
export class CaretColorDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgCaretColor = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgCaretColor() ?? '';
      ...
      this.el.nativeElement.style.caretColor = applied ?? '';

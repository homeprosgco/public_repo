...
export class BorderLeftColorDirective {
  ...
  sgBorderLeftColor = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBorderLeftColor() ?? '';
      ...
      this.el.nativeElement.style.borderLeftColor = applied ?? '';

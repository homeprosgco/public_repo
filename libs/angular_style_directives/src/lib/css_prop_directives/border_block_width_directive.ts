...
@Directive({
  selector: '[sgBorderBlockWidth]',
  standalone: true
})
export class BorderBlockWidthDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgBorderBlockWidth = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBorderBlockWidth() ?? '';
      ...
      this.el.nativeElement.style.borderBlockWidth = applied ?? '';

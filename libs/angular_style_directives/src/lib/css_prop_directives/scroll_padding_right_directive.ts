...
@Directive({
  selector: '[sgScrollPaddingRight]',
  standalone: true
})
export class ScrollPaddingRightDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgScrollPaddingRight = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgScrollPaddingRight() ?? '';
      ...
      this.el.nativeElement.style.scrollPaddingRight = applied ?? '';

...
@Directive({
  selector: '[sgBorderImageSource]',
  standalone: true
})
export class BorderImageSourceDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgBorderImageSource = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBorderImageSource() ?? '';
      ...
      this.el.nativeElement.style.borderImageSource = applied ?? '';

...
@Directive({
  selector: '[sgBorderInlineStartColor]',
  standalone: true
})
export class BorderInlineStartColorDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgBorderInlineStartColor = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBorderInlineStartColor() ?? '';
      ...
      this.el.nativeElement.style.borderInlineStartColor = applied ?? '';

...
@Directive({
  selector: '[sgPaddingInlineStart]',
  standalone: true
})
export class PaddingInlineStartDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgPaddingInlineStart = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgPaddingInlineStart() ?? '';
      ...
      this.el.nativeElement.style.paddingInlineStart = applied ?? '';

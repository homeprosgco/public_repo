...
export class AnimationDelayDirective {
  ...
  sgAnimationDelay = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgAnimationDelay() ?? '';
      ...
      this.el.nativeElement.style.animationDelay = applied ?? '';

...
export class FlexDirectionDirective {
  ...
  sgFlexDirection = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgFlexDirection() ?? '';
      ...
      this.el.nativeElement.style.flexDirection = applied ?? '';

...
@Directive({
  selector: '[sgOutlineOffset]',
  standalone: true
})
export class OutlineOffsetDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgOutlineOffset = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgOutlineOffset() ?? '';
      ...
      this.el.nativeElement.style.outlineOffset = applied ?? '';

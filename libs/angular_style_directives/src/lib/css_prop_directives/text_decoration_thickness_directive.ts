...
@Directive({
  selector: '[sgTextDecorationThickness]',
  standalone: true
})
export class TextDecorationThicknessDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgTextDecorationThickness = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgTextDecorationThickness() ?? '';
      ...
      this.el.nativeElement.style.textDecorationThickness = applied ?? '';

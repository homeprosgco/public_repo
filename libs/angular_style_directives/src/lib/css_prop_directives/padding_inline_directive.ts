...
@Directive({
  selector: '[sgPaddingInline]',
  standalone: true
})
export class PaddingInlineDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgPaddingInline = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgPaddingInline() ?? '';
      ...
      this.el.nativeElement.style.paddingInline = applied ?? '';

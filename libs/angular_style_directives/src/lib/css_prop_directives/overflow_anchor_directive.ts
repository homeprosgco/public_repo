...
@Directive({
  selector: '[sgOverflowAnchor]',
  standalone: true
})
export class OverflowAnchorDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgOverflowAnchor = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgOverflowAnchor() ?? '';
      ...
      this.el.nativeElement.style.overflowAnchor = applied ?? '';

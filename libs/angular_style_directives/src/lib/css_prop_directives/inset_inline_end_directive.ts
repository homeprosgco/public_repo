...
@Directive({
  selector: '[sgInsetInlineEnd]',
  standalone: true
})
export class InsetInlineEndDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgInsetInlineEnd = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgInsetInlineEnd() ?? '';
      ...
      this.el.nativeElement.style.insetInlineEnd = applied ?? '';

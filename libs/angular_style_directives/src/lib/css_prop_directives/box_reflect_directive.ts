...
@Directive({
  selector: '[sgBoxReflect]',
  standalone: true
})
export class BoxReflectDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgBoxReflect = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBoxReflect() ?? '';
      ...
      this.el.nativeElement.style.boxReflect = applied ?? '';

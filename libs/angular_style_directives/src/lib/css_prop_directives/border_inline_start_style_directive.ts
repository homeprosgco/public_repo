...
@Directive({
  selector: '[sgBorderInlineStartStyle]',
  standalone: true
})
export class BorderInlineStartStyleDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgBorderInlineStartStyle = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBorderInlineStartStyle() ?? '';
      ...
      this.el.nativeElement.style.borderInlineStartStyle = applied ?? '';

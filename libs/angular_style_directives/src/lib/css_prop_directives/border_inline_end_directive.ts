...
@Directive({
  selector: '[sgBorderInlineEnd]',
  standalone: true
})
export class BorderInlineEndDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgBorderInlineEnd = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBorderInlineEnd() ?? '';
      ...
      this.el.nativeElement.style.borderInlineEnd = applied ?? '';

...
@Directive({
  selector: '[sgBorderInlineStart]',
  standalone: true
})
export class BorderInlineStartDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgBorderInlineStart = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBorderInlineStart() ?? '';
      ...
      this.el.nativeElement.style.borderInlineStart = applied ?? '';

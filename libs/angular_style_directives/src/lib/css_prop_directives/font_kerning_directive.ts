...
@Directive({
  selector: '[sgFontKerning]',
  standalone: true
})
export class FontKerningDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgFontKerning = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgFontKerning() ?? '';
      ...
      this.el.nativeElement.style.fontKerning = applied ?? '';

...
@Directive({
  selector: '[sgOverscrollBehaviorY]',
  standalone: true
})
export class OverscrollBehaviorYDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgOverscrollBehaviorY = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgOverscrollBehaviorY() ?? '';
      ...
      this.el.nativeElement.style.overscrollBehaviorY = applied ?? '';

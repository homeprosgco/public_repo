...
@Directive({
  selector: '[sgScrollMarginInlineEnd]',
  standalone: true
})
export class ScrollMarginInlineEndDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgScrollMarginInlineEnd = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgScrollMarginInlineEnd() ?? '';
      ...
      this.el.nativeElement.style.scrollMarginInlineEnd = applied ?? '';

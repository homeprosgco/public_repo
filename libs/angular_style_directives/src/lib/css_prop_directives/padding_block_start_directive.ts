...
@Directive({
  selector: '[sgPaddingBlockStart]',
  standalone: true
})
export class PaddingBlockStartDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgPaddingBlockStart = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgPaddingBlockStart() ?? '';
      ...
      this.el.nativeElement.style.paddingBlockStart = applied ?? '';

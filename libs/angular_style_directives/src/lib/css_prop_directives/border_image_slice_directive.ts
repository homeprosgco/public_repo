...
@Directive({
  selector: '[sgBorderImageSlice]',
  standalone: true
})
export class BorderImageSliceDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgBorderImageSlice = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBorderImageSlice() ?? '';
      ...
      this.el.nativeElement.style.borderImageSlice = applied ?? '';

...
@Directive({
  selector: '[sgBackfaceVisibility]',
  standalone: true
})
export class BackfaceVisibilityDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgBackfaceVisibility = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBackfaceVisibility() ?? '';
      ...
      this.el.nativeElement.style.backfaceVisibility = applied ?? '';

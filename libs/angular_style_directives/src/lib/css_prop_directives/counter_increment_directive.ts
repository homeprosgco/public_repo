...
export class CounterIncrementDirective {
  ...
  sgCounterIncrement = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgCounterIncrement() ?? '';
      ...
      this.el.nativeElement.style.counterIncrement = applied ?? '';

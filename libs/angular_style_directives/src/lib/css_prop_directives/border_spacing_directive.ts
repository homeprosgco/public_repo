...
@Directive({
  selector: '[sgBorderSpacing]',
  standalone: true
})
export class BorderSpacingDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgBorderSpacing = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBorderSpacing() ?? '';
      ...
      this.el.nativeElement.style.borderSpacing = applied ?? '';

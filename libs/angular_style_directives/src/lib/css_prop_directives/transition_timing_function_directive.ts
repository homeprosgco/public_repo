...
@Directive({
  selector: '[sgTransitionTimingFunction]',
  standalone: true
})
export class TransitionTimingFunctionDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgTransitionTimingFunction = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgTransitionTimingFunction() ?? '';
      ...
      this.el.nativeElement.style.transitionTimingFunction = applied ?? '';

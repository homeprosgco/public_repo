...
@Directive({
  selector: '[sgScrollMarginLeft]',
  standalone: true
})
export class ScrollMarginLeftDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgScrollMarginLeft = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgScrollMarginLeft() ?? '';
      ...
      this.el.nativeElement.style.scrollMarginLeft = applied ?? '';

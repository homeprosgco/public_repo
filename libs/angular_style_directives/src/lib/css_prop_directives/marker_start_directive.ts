...
@Directive({
  selector: '[sgMarkerStart]',
  standalone: true
})
export class MarkerStartDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgMarkerStart = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgMarkerStart() ?? '';
      ...
      this.el.nativeElement.style.markerStart = applied ?? '';

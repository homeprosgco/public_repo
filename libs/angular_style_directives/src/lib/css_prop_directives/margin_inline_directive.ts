...
@Directive({
  selector: '[sgMarginInline]',
  standalone: true
})
export class MarginInlineDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgMarginInline = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgMarginInline() ?? '';
      ...
      this.el.nativeElement.style.marginInline = applied ?? '';

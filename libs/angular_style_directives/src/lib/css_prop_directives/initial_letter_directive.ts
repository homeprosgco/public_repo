...
@Directive({
  selector: '[sgInitialLetter]',
  standalone: true
})
export class InitialLetterDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgInitialLetter = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgInitialLetter() ?? '';
      ...
      this.el.nativeElement.style.initialLetter = applied ?? '';

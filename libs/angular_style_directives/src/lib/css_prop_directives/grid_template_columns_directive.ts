...
@Directive({
  selector: '[sgGridTemplateColumns]',
  standalone: true
})
export class GridTemplateColumnsDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgGridTemplateColumns = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgGridTemplateColumns() ?? '';
      ...
      this.el.nativeElement.style.gridTemplateColumns = applied ?? '';

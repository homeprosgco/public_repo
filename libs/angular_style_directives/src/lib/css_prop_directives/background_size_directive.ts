...
export class BackgroundSizeDirective {
  ...
  sgBackgroundSize = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBackgroundSize() ?? '';
      ...
      this.el.nativeElement.style.backgroundSize = applied ?? '';

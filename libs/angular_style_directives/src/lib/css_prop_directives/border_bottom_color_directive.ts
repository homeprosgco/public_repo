...
export class BorderBottomColorDirective {
  ...
  sgBorderBottomColor = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBorderBottomColor() ?? '';
      ...
      this.el.nativeElement.style.borderBottomColor = applied ?? '';

...
@Directive({
  selector: '[sgInsetInline]',
  standalone: true
})
export class InsetInlineDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgInsetInline = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgInsetInline() ?? '';
      ...
      this.el.nativeElement.style.insetInline = applied ?? '';

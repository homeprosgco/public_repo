...
@Directive({
  selector: '[sgScrollSnapAlign]',
  standalone: true
})
export class ScrollSnapAlignDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgScrollSnapAlign = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgScrollSnapAlign() ?? '';
      ...
      this.el.nativeElement.style.scrollSnapAlign = applied ?? '';

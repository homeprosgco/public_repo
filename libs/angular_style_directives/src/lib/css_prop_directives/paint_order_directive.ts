...
@Directive({
  selector: '[sgPaintOrder]',
  standalone: true
})
export class PaintOrderDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgPaintOrder = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgPaintOrder() ?? '';
      ...
      this.el.nativeElement.style.paintOrder = applied ?? '';

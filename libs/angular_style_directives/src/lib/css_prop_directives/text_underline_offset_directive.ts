...
@Directive({
  selector: '[sgTextUnderlineOffset]',
  standalone: true
})
export class TextUnderlineOffsetDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgTextUnderlineOffset = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgTextUnderlineOffset() ?? '';
      ...
      this.el.nativeElement.style.textUnderlineOffset = applied ?? '';

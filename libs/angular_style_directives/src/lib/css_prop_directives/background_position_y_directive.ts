...
@Directive({
  selector: '[sgBackgroundPositionY]',
  standalone: true
})
export class BackgroundPositionYDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgBackgroundPositionY = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBackgroundPositionY() ?? '';
      ...
      this.el.nativeElement.style.backgroundPositionY = applied ?? '';

...
@Directive({
  selector: '[sgBackgroundBlendMode]',
  standalone: true
})
export class BackgroundBlendModeDirective {
  ...
  sgBackgroundBlendMode = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBackgroundBlendMode() ?? '';
      ...
      this.el.nativeElement.style.backgroundBlendMode = applied ?? '';

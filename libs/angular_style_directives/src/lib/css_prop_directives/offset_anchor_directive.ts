...
@Directive({
  selector: '[sgOffsetAnchor]',
  standalone: true
})
export class OffsetAnchorDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgOffsetAnchor = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgOffsetAnchor() ?? '';
      ...
      this.el.nativeElement.style.offsetAnchor = applied ?? '';

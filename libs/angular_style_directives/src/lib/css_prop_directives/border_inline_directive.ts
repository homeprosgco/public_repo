...
@Directive({
  selector: '[sgBorderInline]',
  standalone: true
})
export class BorderInlineDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgBorderInline = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBorderInline() ?? '';
      ...
      this.el.nativeElement.style.borderInline = applied ?? '';

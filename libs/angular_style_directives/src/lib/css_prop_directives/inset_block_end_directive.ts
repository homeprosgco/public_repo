...
@Directive({
  selector: '[sgInsetBlockEnd]',
  standalone: true
})
export class InsetBlockEndDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgInsetBlockEnd = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgInsetBlockEnd() ?? '';
      ...
      this.el.nativeElement.style.insetBlockEnd = applied ?? '';

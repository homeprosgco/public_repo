...
@Directive({
  selector: '[sgBackgroundOrigin]',
  standalone: true
})
export class BackgroundOriginDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgBackgroundOrigin = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBackgroundOrigin() ?? '';
      ...
      this.el.nativeElement.style.backgroundOrigin = applied ?? '';

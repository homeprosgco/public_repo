...
@Directive({
  selector: '[sgBackgroundAttachment]',
  standalone: true
})
export class BackgroundAttachmentDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgBackgroundAttachment = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBackgroundAttachment() ?? '';
      ...
      this.el.nativeElement.style.backgroundAttachment = applied ?? '';

...
@Directive({
  selector: '[sgListStyleImage]',
  standalone: true
})
export class ListStyleImageDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgListStyleImage = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgListStyleImage() ?? '';
      ...
      this.el.nativeElement.style.listStyleImage = applied ?? '';

...
@Directive({
  selector: '[sgCounterSet]',
  standalone: true
})
export class CounterSetDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgCounterSet = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgCounterSet() ?? '';
      ...
      this.el.nativeElement.style.counterSet = applied ?? '';

...
@Directive({
  selector: '[sgBorderBlock]',
  standalone: true
})
export class BorderBlockDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgBorderBlock = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBorderBlock() ?? '';
      ...
      this.el.nativeElement.style.borderBlock = applied ?? '';

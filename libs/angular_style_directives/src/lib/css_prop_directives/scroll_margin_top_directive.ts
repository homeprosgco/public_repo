...
@Directive({
  selector: '[sgScrollMarginTop]',
  standalone: true
})
export class ScrollMarginTopDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgScrollMarginTop = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgScrollMarginTop() ?? '';
      ...
      this.el.nativeElement.style.scrollMarginTop = applied ?? '';

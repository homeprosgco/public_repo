...
@Directive({
  selector: '[sgLetterSpacing]',
  standalone: true
})
export class LetterSpacingDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgLetterSpacing = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgLetterSpacing() ?? '';
      ...
      this.el.nativeElement.style.letterSpacing = applied ?? '';

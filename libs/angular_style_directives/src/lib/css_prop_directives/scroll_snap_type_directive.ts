...
@Directive({
  selector: '[sgScrollSnapType]',
  standalone: true
})
export class ScrollSnapTypeDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgScrollSnapType = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgScrollSnapType() ?? '';
      ...
      this.el.nativeElement.style.scrollSnapType = applied ?? '';

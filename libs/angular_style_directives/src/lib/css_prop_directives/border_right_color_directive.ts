...
export class BorderRightColorDirective {
  ...
  sgBorderRightColor = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBorderRightColor() ?? '';
      ...
      this.el.nativeElement.style.borderRightColor = applied ?? '';

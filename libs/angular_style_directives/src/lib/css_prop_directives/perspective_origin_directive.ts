...
@Directive({
  selector: '[sgPerspectiveOrigin]',
  standalone: true
})
export class PerspectiveOriginDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgPerspectiveOrigin = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgPerspectiveOrigin() ?? '';
      ...
      this.el.nativeElement.style.perspectiveOrigin = applied ?? '';

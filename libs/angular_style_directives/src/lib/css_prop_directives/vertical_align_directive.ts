...
export class VerticalAlignDirective {
  ...
  sgVerticalAlign = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgVerticalAlign() ?? '';
      ...
      this.el.nativeElement.style.verticalAlign = applied ?? '';

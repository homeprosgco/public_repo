...
@Directive({
  selector: '[sgBlockSize]',
  standalone: true
})
export class BlockSizeDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgBlockSize = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBlockSize() ?? '';
      ...
      this.el.nativeElement.style.blockSize = applied ?? '';

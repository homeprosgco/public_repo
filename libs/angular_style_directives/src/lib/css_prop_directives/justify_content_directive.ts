...
@Directive({
  selector: '[sgJustifyContent]',
  standalone: true
})
export class JustifyContentDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgJustifyContent = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgJustifyContent() ?? '';
      ...
      this.el.nativeElement.style.justifyContent = applied ?? '';

...
@Directive({
  selector: '[sgBorderBlockStartWidth]',
  standalone: true
})
export class BorderBlockStartWidthDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgBorderBlockStartWidth = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBorderBlockStartWidth() ?? '';
      ...
      this.el.nativeElement.style.borderBlockStartWidth = applied ?? '';

...
@Directive({
  selector: '[sgAnimationDirection]',
  standalone: true
})
export class AnimationDirectionDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgAnimationDirection = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgAnimationDirection() ?? '';
      ...
      this.el.nativeElement.style.animationDirection = applied ?? '';

...
@Directive({
  selector: '[sgGridTemplateAreas]',
  standalone: true
})
export class GridTemplateAreasDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgGridTemplateAreas = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgGridTemplateAreas() ?? '';
      ...
      this.el.nativeElement.style.gridTemplateAreas = applied ?? '';

...
export class BorderTopWidthDirective {
  ...
  sgBorderTopWidth = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBorderTopWidth() ?? '';
      ...
      this.el.nativeElement.style.borderTopWidth = applied ?? '';

...
@Directive({
  selector: '[sgBreakAfter]',
  standalone: true
})
export class BreakAfterDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgBreakAfter = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBreakAfter() ?? '';
      ...
      this.el.nativeElement.style.breakAfter = applied ?? '';

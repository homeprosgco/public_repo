...
@Directive({
  selector: '[sgScrollMarginInlineStart]',
  standalone: true
})
export class ScrollMarginInlineStartDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgScrollMarginInlineStart = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgScrollMarginInlineStart() ?? '';
      ...
      this.el.nativeElement.style.scrollMarginInlineStart = applied ?? '';

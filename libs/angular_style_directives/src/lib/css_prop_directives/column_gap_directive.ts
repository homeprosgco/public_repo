...
@Directive({
  selector: '[sgColumnGap]',
  standalone: true
})
export class ColumnGapDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgColumnGap = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgColumnGap() ?? '';
      ...
      this.el.nativeElement.style.columnGap = applied ?? '';

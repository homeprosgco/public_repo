...
@Directive({
  selector: '[sgGridColumnStart]',
  standalone: true
})
export class GridColumnStartDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgGridColumnStart = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgGridColumnStart() ?? '';
      ...
      this.el.nativeElement.style.gridColumnStart = applied ?? '';

...
@Directive({
  selector: '[sgPaddingInlineEnd]',
  standalone: true
})
export class PaddingInlineEndDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgPaddingInlineEnd = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgPaddingInlineEnd() ?? '';
      ...
      this.el.nativeElement.style.paddingInlineEnd = applied ?? '';

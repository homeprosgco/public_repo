...
@Directive({
  selector: '[sgBorderTopRightRadius]',
  standalone: true
})
export class BorderTopRightRadiusDirective {
  ...
  sgBorderTopRightRadius = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBorderTopRightRadius() ?? '';
      ...
      this.el.nativeElement.style.borderTopRightRadius = applied ?? '';

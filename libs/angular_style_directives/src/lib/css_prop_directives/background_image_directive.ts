...
export class BackgroundImageDirective {
  ...
  sgBackgroundImage = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBackgroundImage() ?? '';
      ...
      this.el.nativeElement.style.backgroundImage = applied ?? '';

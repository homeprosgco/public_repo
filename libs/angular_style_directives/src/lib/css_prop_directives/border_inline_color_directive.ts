...
@Directive({
  selector: '[sgBorderInlineColor]',
  standalone: true
})
export class BorderInlineColorDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgBorderInlineColor = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBorderInlineColor() ?? '';
      ...
      this.el.nativeElement.style.borderInlineColor = applied ?? '';

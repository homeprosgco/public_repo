...
@Directive({
  selector: '[sgUnicodeBidi]',
  standalone: true
})
export class UnicodeBidiDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgUnicodeBidi = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgUnicodeBidi() ?? '';
      ...
      this.el.nativeElement.style.unicodeBidi = applied ?? '';

...
@Directive({
  selector: '[sgPaddingBlockEnd]',
  standalone: true
})
export class PaddingBlockEndDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgPaddingBlockEnd = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgPaddingBlockEnd() ?? '';
      ...
      this.el.nativeElement.style.paddingBlockEnd = applied ?? '';

...
@Directive({
  selector: '[sgWordBreak]',
  standalone: true
})
export class WordBreakDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgWordBreak = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgWordBreak() ?? '';
      ...
      this.el.nativeElement.style.wordBreak = applied ?? '';

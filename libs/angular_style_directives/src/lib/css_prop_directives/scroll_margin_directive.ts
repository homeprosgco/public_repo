...
@Directive({
  selector: '[sgScrollMargin]',
  standalone: true
})
export class ScrollMarginDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgScrollMargin = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgScrollMargin() ?? '';
      ...
      this.el.nativeElement.style.scrollMargin = applied ?? '';

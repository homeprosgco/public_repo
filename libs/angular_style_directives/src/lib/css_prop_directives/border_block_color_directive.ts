...
@Directive({
  selector: '[sgBorderBlockColor]',
  standalone: true
})
export class BorderBlockColorDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgBorderBlockColor = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBorderBlockColor() ?? '';
      ...
      this.el.nativeElement.style.borderBlockColor = applied ?? '';

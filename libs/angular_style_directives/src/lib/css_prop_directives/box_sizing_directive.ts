...
@Directive({
  selector: '[sgBoxSizing]',
  standalone: true
})
export class BoxSizingDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgBoxSizing = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBoxSizing() ?? '';
      ...
      this.el.nativeElement.style.boxSizing = applied ?? '';

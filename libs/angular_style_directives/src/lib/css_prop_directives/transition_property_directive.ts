...
@Directive({
  selector: '[sgTransitionProperty]',
  standalone: true
})
export class TransitionPropertyDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgTransitionProperty = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgTransitionProperty() ?? '';
      ...
      this.el.nativeElement.style.transitionProperty = applied ?? '';

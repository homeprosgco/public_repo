...
@Directive({
  selector: '[sgGridRowEnd]',
  standalone: true
})
export class GridRowEndDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgGridRowEnd = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgGridRowEnd() ?? '';
      ...
      this.el.nativeElement.style.gridRowEnd = applied ?? '';

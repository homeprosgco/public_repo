...
@Directive({
  selector: '[sgOutlineColor]',
  standalone: true
})
export class OutlineColorDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgOutlineColor = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgOutlineColor() ?? '';
      ...
      this.el.nativeElement.style.outlineColor = applied ?? '';

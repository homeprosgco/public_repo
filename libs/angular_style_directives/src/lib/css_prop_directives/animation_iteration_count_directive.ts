...
@Directive({
  selector: '[sgAnimationIterationCount]',
  standalone: true
})
export class AnimationIterationCountDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgAnimationIterationCount = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgAnimationIterationCount() ?? '';
      ...
      this.el.nativeElement.style.animationIterationCount = applied ?? '';

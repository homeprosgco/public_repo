...
@Directive({
  selector: '[sgTextDecorationStyle]',
  standalone: true
})
export class TextDecorationStyleDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgTextDecorationStyle = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgTextDecorationStyle() ?? '';
      ...
      this.el.nativeElement.style.textDecorationStyle = applied ?? '';

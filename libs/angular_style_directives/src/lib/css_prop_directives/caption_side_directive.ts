...
@Directive({
  selector: '[sgCaptionSide]',
  standalone: true
})
export class CaptionSideDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgCaptionSide = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgCaptionSide() ?? '';
      ...
      this.el.nativeElement.style.captionSide = applied ?? '';

...
@Directive({
  selector: '[sgWordSpacing]',
  standalone: true
})
export class WordSpacingDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgWordSpacing = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgWordSpacing() ?? '';
      ...
      this.el.nativeElement.style.wordSpacing = applied ?? '';

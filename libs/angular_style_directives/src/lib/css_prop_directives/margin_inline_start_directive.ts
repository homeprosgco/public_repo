...
@Directive({
  selector: '[sgMarginInlineStart]',
  standalone: true
})
export class MarginInlineStartDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgMarginInlineStart = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgMarginInlineStart() ?? '';
      ...
      this.el.nativeElement.style.marginInlineStart = applied ?? '';

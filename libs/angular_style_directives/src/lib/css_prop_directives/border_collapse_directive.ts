...
@Directive({
  selector: '[sgBorderCollapse]',
  standalone: true
})
export class BorderCollapseDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgBorderCollapse = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBorderCollapse() ?? '';
      ...
      this.el.nativeElement.style.borderCollapse = applied ?? '';

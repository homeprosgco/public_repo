...
@Directive({
  selector: '[sgTextDecorationLine]',
  standalone: true
})
export class TextDecorationLineDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgTextDecorationLine = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgTextDecorationLine() ?? '';
      ...
      this.el.nativeElement.style.textDecorationLine = applied ?? '';

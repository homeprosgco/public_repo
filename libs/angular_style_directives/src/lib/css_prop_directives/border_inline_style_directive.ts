...
@Directive({
  selector: '[sgBorderInlineStyle]',
  standalone: true
})
export class BorderInlineStyleDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgBorderInlineStyle = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBorderInlineStyle() ?? '';
      ...
      this.el.nativeElement.style.borderInlineStyle = applied ?? '';

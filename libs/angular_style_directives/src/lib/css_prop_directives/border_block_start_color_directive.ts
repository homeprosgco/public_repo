...
@Directive({
  selector: '[sgBorderBlockStartColor]',
  standalone: true
})
export class BorderBlockStartColorDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgBorderBlockStartColor = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBorderBlockStartColor() ?? '';
      ...
      this.el.nativeElement.style.borderBlockStartColor = applied ?? '';

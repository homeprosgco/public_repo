...
@Directive({
  selector: '[sgBorderRightWidth]',
  standalone: true
})
export class BorderRightWidthDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgBorderRightWidth = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBorderRightWidth() ?? '';
      ...
      this.el.nativeElement.style.borderRightWidth = applied ?? '';

...
@Directive({
  selector: '[sgTextUnderlinePosition]',
  standalone: true
})
export class TextUnderlinePositionDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgTextUnderlinePosition = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgTextUnderlinePosition() ?? '';
      ...
      this.el.nativeElement.style.textUnderlinePosition = applied ?? '';

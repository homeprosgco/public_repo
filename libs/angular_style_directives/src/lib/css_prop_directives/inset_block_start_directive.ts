...
@Directive({
  selector: '[sgInsetBlockStart]',
  standalone: true
})
export class InsetBlockStartDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgInsetBlockStart = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgInsetBlockStart() ?? '';
      ...
      this.el.nativeElement.style.insetBlockStart = applied ?? '';

...
@Directive({
  selector: '[sgPageBreakInside]',
  standalone: true
})
export class PageBreakInsideDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgPageBreakInside = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgPageBreakInside() ?? '';
      ...
      this.el.nativeElement.style.pageBreakInside = applied ?? '';

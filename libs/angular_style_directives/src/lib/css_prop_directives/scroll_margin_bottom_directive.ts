...
@Directive({
  selector: '[sgScrollMarginBottom]',
  standalone: true
})
export class ScrollMarginBottomDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgScrollMarginBottom = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgScrollMarginBottom() ?? '';
      ...
      this.el.nativeElement.style.scrollMarginBottom = applied ?? '';

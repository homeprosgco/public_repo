...
export class BorderLeftWidthDirective {
  ...
  sgBorderLeftWidth = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBorderLeftWidth() ?? '';
      ...
      this.el.nativeElement.style.borderLeftWidth = applied ?? '';

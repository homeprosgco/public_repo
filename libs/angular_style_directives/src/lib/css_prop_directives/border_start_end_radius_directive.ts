...
@Directive({
  selector: '[sgBorderStartEndRadius]',
  standalone: true
})
export class BorderStartEndRadiusDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgBorderStartEndRadius = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBorderStartEndRadius() ?? '';
      ...
      this.el.nativeElement.style.borderStartEndRadius = applied ?? '';

...
@Directive({
  selector: '[sgScrollbarColor]',
  standalone: true
})
export class ScrollbarColorDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgScrollbarColor = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgScrollbarColor() ?? '';
      ...
      this.el.nativeElement.style.scrollbarColor = applied ?? '';

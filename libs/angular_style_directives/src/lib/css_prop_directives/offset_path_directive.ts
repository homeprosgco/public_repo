...
@Directive({
  selector: '[sgOffsetPath]',
  standalone: true
})
export class OffsetPathDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgOffsetPath = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgOffsetPath() ?? '';
      ...
      this.el.nativeElement.style.offsetPath = applied ?? '';

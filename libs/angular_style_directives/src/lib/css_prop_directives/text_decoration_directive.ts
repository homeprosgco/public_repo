...
export class TextDecorationDirective {
  ...
  sgTextDecoration = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgTextDecoration() ?? '';
      ...
      this.el.nativeElement.style.textDecoration = applied ?? '';

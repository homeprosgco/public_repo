...
@Directive({
  selector: '[sgOutlineStyle]',
  standalone: true
})
export class OutlineStyleDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgOutlineStyle = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgOutlineStyle() ?? '';
      ...
      this.el.nativeElement.style.outlineStyle = applied ?? '';

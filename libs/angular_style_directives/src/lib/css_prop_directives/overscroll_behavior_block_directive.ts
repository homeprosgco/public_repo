...
@Directive({
  selector: '[sgOverscrollBehaviorBlock]',
  standalone: true
})
export class OverscrollBehaviorBlockDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgOverscrollBehaviorBlock = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgOverscrollBehaviorBlock() ?? '';
      ...
      this.el.nativeElement.style.overscrollBehaviorBlock = applied ?? '';

...
@Directive({
  selector: '[sgFontVariantNumeric]',
  standalone: true
})
export class FontVariantNumericDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgFontVariantNumeric = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgFontVariantNumeric() ?? '';
      ...
      this.el.nativeElement.style.fontVariantNumeric = applied ?? '';

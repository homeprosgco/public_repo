...
@Directive({
  selector: '[sgHangingPunctuation]',
  standalone: true
})
export class HangingPunctuationDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgHangingPunctuation = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgHangingPunctuation() ?? '';
      ...
      this.el.nativeElement.style.hangingPunctuation = applied ?? '';

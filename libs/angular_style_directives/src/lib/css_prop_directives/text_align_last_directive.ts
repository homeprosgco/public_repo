...
@Directive({
  selector: '[sgTextAlignLast]',
  standalone: true
})
export class TextAlignLastDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgTextAlignLast = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgTextAlignLast() ?? '';
      ...
      this.el.nativeElement.style.textAlignLast = applied ?? '';

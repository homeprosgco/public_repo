...
@Directive({
  selector: '[sgScrollBehavior]',
  standalone: true
})
export class ScrollBehaviorDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgScrollBehavior = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgScrollBehavior() ?? '';
      ...
      this.el.nativeElement.style.scrollBehavior = applied ?? '';

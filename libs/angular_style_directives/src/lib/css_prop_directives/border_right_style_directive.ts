...
@Directive({
  selector: '[sgBorderRightStyle]',
  standalone: true
})
export class BorderRightStyleDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgBorderRightStyle = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBorderRightStyle() ?? '';
      ...
      this.el.nativeElement.style.borderRightStyle = applied ?? '';

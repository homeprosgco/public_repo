...
export class BorderBottomWidthDirective {
  ...
  sgBorderBottomWidth = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBorderBottomWidth() ?? '';
      ...
      this.el.nativeElement.style.borderBottomWidth = applied ?? '';

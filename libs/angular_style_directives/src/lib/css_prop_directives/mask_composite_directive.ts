...
@Directive({
  selector: '[sgMaskComposite]',
  standalone: true
})
export class MaskCompositeDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgMaskComposite = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgMaskComposite() ?? '';
      ...
      this.el.nativeElement.style.maskComposite = applied ?? '';

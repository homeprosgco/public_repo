...
@Directive({
  selector: '[sgPageBreakBefore]',
  standalone: true
})
export class PageBreakBeforeDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgPageBreakBefore = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgPageBreakBefore() ?? '';
      ...
      this.el.nativeElement.style.pageBreakBefore = applied ?? '';

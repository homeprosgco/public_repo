...
@Directive({
  selector: '[sgInlineSize]',
  standalone: true
})
export class InlineSizeDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgInlineSize = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgInlineSize() ?? '';
      ...
      this.el.nativeElement.style.inlineSize = applied ?? '';

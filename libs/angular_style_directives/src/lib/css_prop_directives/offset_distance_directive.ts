...
@Directive({
  selector: '[sgOffsetDistance]',
  standalone: true
})
export class OffsetDistanceDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgOffsetDistance = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgOffsetDistance() ?? '';
      ...
      this.el.nativeElement.style.offsetDistance = applied ?? '';

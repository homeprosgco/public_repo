...
@Directive({
  selector: '[sgBackdropFilter]',
  standalone: true
})
export class BackdropFilterDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgBackdropFilter = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBackdropFilter() ?? '';
      ...
      this.el.nativeElement.style.backdropFilter = applied ?? '';

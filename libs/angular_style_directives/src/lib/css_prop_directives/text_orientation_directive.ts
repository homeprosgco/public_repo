...
@Directive({
  selector: '[sgTextOrientation]',
  standalone: true
})
export class TextOrientationDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgTextOrientation = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgTextOrientation() ?? '';
      ...
      this.el.nativeElement.style.textOrientation = applied ?? '';

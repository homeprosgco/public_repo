...
@Directive({
  selector: '[sgOffsetRotate]',
  standalone: true
})
export class OffsetRotateDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgOffsetRotate = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgOffsetRotate() ?? '';
      ...
      this.el.nativeElement.style.offsetRotate = applied ?? '';

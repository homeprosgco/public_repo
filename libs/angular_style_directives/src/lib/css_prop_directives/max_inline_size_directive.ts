...
@Directive({
  selector: '[sgMaxInlineSize]',
  standalone: true
})
export class MaxInlineSizeDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgMaxInlineSize = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgMaxInlineSize() ?? '';
      ...
      this.el.nativeElement.style.maxInlineSize = applied ?? '';

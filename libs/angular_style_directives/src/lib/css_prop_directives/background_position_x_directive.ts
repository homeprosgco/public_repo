...
@Directive({
  selector: '[sgBackgroundPositionX]',
  standalone: true
})
export class BackgroundPositionXDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgBackgroundPositionX = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBackgroundPositionX() ?? '';
      ...
      this.el.nativeElement.style.backgroundPositionX = applied ?? '';

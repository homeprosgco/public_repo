...
@Directive({
  selector: '[sgBorderBottomRightRadius]',
  standalone: true
})
export class BorderBottomRightRadiusDirective {
  ...
  sgBorderBottomRightRadius = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBorderBottomRightRadius() ?? '';
      ...
      this.el.nativeElement.style.borderBottomRightRadius = applied ?? '';

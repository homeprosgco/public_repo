...
@Directive({
  selector: '[sgInsetInlineStart]',
  standalone: true
})
export class InsetInlineStartDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgInsetInlineStart = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgInsetInlineStart() ?? '';
      ...
      this.el.nativeElement.style.insetInlineStart = applied ?? '';

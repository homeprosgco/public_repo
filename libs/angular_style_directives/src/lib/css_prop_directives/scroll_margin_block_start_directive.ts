...
@Directive({
  selector: '[sgScrollMarginBlockStart]',
  standalone: true
})
export class ScrollMarginBlockStartDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgScrollMarginBlockStart = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgScrollMarginBlockStart() ?? '';
      ...
      this.el.nativeElement.style.scrollMarginBlockStart = applied ?? '';

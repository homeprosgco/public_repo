...
@Directive({
  selector: '[sgOverscrollBehaviorX]',
  standalone: true
})
export class OverscrollBehaviorXDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgOverscrollBehaviorX = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgOverscrollBehaviorX() ?? '';
      ...
      this.el.nativeElement.style.overscrollBehaviorX = applied ?? '';

...
@Directive({
  selector: '[sgMaskRepeat]',
  standalone: true
})
export class MaskRepeatDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgMaskRepeat = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgMaskRepeat() ?? '';
      ...
      this.el.nativeElement.style.maskRepeat = applied ?? '';

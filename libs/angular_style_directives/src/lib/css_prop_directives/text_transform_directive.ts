...
export class TextTransformDirective {
  ...
  sgTextTransform = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgTextTransform() ?? '';
      ...
      this.el.nativeElement.style.textTransform = applied ?? '';

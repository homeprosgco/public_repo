...
@Directive({
  selector: '[sgJustifySelf]',
  standalone: true
})
export class JustifySelfDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgJustifySelf = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgJustifySelf() ?? '';
      ...
      this.el.nativeElement.style.justifySelf = applied ?? '';

...
@Directive({
  selector: '[sgPaddingBlock]',
  standalone: true
})
export class PaddingBlockDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgPaddingBlock = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgPaddingBlock() ?? '';
      ...
      this.el.nativeElement.style.paddingBlock = applied ?? '';

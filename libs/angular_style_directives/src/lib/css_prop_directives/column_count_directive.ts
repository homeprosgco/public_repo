...
@Directive({
  selector: '[sgColumnCount]',
  standalone: true
})
export class ColumnCountDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgColumnCount = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgColumnCount() ?? '';
      ...
      this.el.nativeElement.style.columnCount = applied ?? '';

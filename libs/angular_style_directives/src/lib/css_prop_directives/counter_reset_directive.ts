...
@Directive({
  selector: '[sgCounterReset]',
  standalone: true
})
export class CounterResetDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgCounterReset = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgCounterReset() ?? '';
      ...
      this.el.nativeElement.style.counterReset = applied ?? '';

...
@Directive({
  selector: '[sgScrollMarginBlockEnd]',
  standalone: true
})
export class ScrollMarginBlockEndDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgScrollMarginBlockEnd = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgScrollMarginBlockEnd() ?? '';
      ...
      this.el.nativeElement.style.scrollMarginBlockEnd = applied ?? '';

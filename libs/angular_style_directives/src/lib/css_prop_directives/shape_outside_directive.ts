...
@Directive({
  selector: '[sgShapeOutside]',
  standalone: true
})
export class ShapeOutsideDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgShapeOutside = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgShapeOutside() ?? '';
      ...
      this.el.nativeElement.style.shapeOutside = applied ?? '';

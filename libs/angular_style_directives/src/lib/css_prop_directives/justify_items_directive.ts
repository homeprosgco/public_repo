...
@Directive({
  selector: '[sgJustifyItems]',
  standalone: true
})
export class JustifyItemsDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgJustifyItems = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgJustifyItems() ?? '';
      ...
      this.el.nativeElement.style.justifyItems = applied ?? '';

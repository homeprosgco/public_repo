...
@Directive({
  selector: '[sgAnimationFillMode]',
  standalone: true
})
export class AnimationFillModeDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgAnimationFillMode = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgAnimationFillMode() ?? '';
      ...
      this.el.nativeElement.style.animationFillMode = applied ?? '';

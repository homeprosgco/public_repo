...
@Directive({
  selector: '[sgAnimationName]',
  standalone: true
})
export class AnimationNameDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgAnimationName = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgAnimationName() ?? '';
      ...
      this.el.nativeElement.style.animationName = applied ?? '';

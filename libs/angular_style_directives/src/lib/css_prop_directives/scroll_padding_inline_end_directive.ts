...
@Directive({
  selector: '[sgScrollPaddingInlineEnd]',
  standalone: true
})
export class ScrollPaddingInlineEndDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgScrollPaddingInlineEnd = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgScrollPaddingInlineEnd() ?? '';
      ...
      this.el.nativeElement.style.scrollPaddingInlineEnd = applied ?? '';

...
@Directive({
  selector: '[sgBorderEndEndRadius]',
  standalone: true
})
export class BorderEndEndRadiusDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgBorderEndEndRadius = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBorderEndEndRadius() ?? '';
      ...
      this.el.nativeElement.style.borderEndEndRadius = applied ?? '';

...
@Directive({
  selector: '[sgAlignContent]',
  standalone: true
})
export class AlignContentDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgAlignContent = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgAlignContent() ?? '';
      ...
      this.el.nativeElement.style.alignContent = applied ?? '';

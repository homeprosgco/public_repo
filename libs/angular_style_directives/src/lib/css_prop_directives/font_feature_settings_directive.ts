...
@Directive({
  selector: '[sgFontFeatureSettings]',
  standalone: true
})
export class FontFeatureSettingsDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgFontFeatureSettings = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgFontFeatureSettings() ?? '';
      ...
      this.el.nativeElement.style.fontFeatureSettings = applied ?? '';

...
@Directive({
  selector: '[sgColumnRuleStyle]',
  standalone: true
})
export class ColumnRuleStyleDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgColumnRuleStyle = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgColumnRuleStyle() ?? '';
      ...
      this.el.nativeElement.style.columnRuleStyle = applied ?? '';

...
@Directive({
  selector: '[sgMixBlendMode]',
  standalone: true
})
export class MixBlendModeDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgMixBlendMode = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgMixBlendMode() ?? '';
      ...
      this.el.nativeElement.style.mixBlendMode = applied ?? '';

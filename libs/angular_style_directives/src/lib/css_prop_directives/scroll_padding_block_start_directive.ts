...
@Directive({
  selector: '[sgScrollPaddingBlockStart]',
  standalone: true
})
export class ScrollPaddingBlockStartDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgScrollPaddingBlockStart = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgScrollPaddingBlockStart() ?? '';
      ...
      this.el.nativeElement.style.scrollPaddingBlockStart = applied ?? '';

...
@Directive({
  selector: '[sgListStyle]',
  standalone: true
})
export class ListStyleDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgListStyle = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgListStyle() ?? '';
      ...
      this.el.nativeElement.style.listStyle = applied ?? '';

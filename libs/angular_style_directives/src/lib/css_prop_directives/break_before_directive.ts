...
@Directive({
  selector: '[sgBreakBefore]',
  standalone: true
})
export class BreakBeforeDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgBreakBefore = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBreakBefore() ?? '';
      ...
      this.el.nativeElement.style.breakBefore = applied ?? '';

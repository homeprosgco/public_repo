...
@Directive({
  selector: '[sgMarginInlineEnd]',
  standalone: true
})
export class MarginInlineEndDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgMarginInlineEnd = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgMarginInlineEnd() ?? '';
      ...
      this.el.nativeElement.style.marginInlineEnd = applied ?? '';

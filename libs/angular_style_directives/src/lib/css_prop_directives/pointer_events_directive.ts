...
export class PointerEventsDirective {
  ...
  sgPointerEvents = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgPointerEvents() ?? '';
      ...
      this.el.nativeElement.style.pointerEvents = applied ?? '';

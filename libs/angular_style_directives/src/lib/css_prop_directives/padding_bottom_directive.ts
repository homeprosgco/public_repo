...
export class PaddingBottomDirective {
  ...
  sgPaddingBottom = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgPaddingBottom() ?? '';
      ...
      this.el.nativeElement.style.paddingBottom = applied ?? '';

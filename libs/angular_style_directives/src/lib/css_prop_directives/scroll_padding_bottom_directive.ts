...
@Directive({
  selector: '[sgScrollPaddingBottom]',
  standalone: true
})
export class ScrollPaddingBottomDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgScrollPaddingBottom = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgScrollPaddingBottom() ?? '';
      ...
      this.el.nativeElement.style.scrollPaddingBottom = applied ?? '';

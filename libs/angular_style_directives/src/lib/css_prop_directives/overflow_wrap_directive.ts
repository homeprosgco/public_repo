...
@Directive({
  selector: '[sgOverflowWrap]',
  standalone: true
})
export class OverflowWrapDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgOverflowWrap = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgOverflowWrap() ?? '';
      ...
      this.el.nativeElement.style.overflowWrap = applied ?? '';

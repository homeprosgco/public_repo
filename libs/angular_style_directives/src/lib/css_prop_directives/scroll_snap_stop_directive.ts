...
@Directive({
  selector: '[sgScrollSnapStop]',
  standalone: true
})
export class ScrollSnapStopDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgScrollSnapStop = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgScrollSnapStop() ?? '';
      ...
      this.el.nativeElement.style.scrollSnapStop = applied ?? '';

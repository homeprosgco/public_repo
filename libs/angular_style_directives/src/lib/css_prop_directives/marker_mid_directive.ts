...
@Directive({
  selector: '[sgMarkerMid]',
  standalone: true
})
export class MarkerMidDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgMarkerMid = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgMarkerMid() ?? '';
      ...
      this.el.nativeElement.style.markerMid = applied ?? '';

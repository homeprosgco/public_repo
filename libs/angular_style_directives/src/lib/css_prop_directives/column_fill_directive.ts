...
@Directive({
  selector: '[sgColumnFill]',
  standalone: true
})
export class ColumnFillDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgColumnFill = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgColumnFill() ?? '';
      ...
      this.el.nativeElement.style.columnFill = applied ?? '';

...
@Directive({
  selector: '[sgScrollMarginInline]',
  standalone: true
})
export class ScrollMarginInlineDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgScrollMarginInline = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgScrollMarginInline() ?? '';
      ...
      this.el.nativeElement.style.scrollMarginInline = applied ?? '';

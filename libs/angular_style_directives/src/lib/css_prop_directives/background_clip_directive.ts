...
@Directive({
  selector: '[sgBackgroundClip]',
  standalone: true
})
export class BackgroundClipDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgBackgroundClip = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBackgroundClip() ?? '';
      ...
      this.el.nativeElement.style.backgroundClip = applied ?? '';

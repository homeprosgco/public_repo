...
@Directive({
  selector: '[sgBorderInlineWidth]',
  standalone: true
})
export class BorderInlineWidthDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgBorderInlineWidth = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBorderInlineWidth() ?? '';
      ...
      this.el.nativeElement.style.borderInlineWidth = applied ?? '';

...
@Directive({
  selector: '[sgMaxBlockSize]',
  standalone: true
})
export class MaxBlockSizeDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgMaxBlockSize = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgMaxBlockSize() ?? '';
      ...
      this.el.nativeElement.style.maxBlockSize = applied ?? '';

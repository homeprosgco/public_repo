...
@Directive({
  selector: '[sgBoxDecorationBreak]',
  standalone: true
})
export class BoxDecorationBreakDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgBoxDecorationBreak = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBoxDecorationBreak() ?? '';
      ...
      this.el.nativeElement.style.boxDecorationBreak = applied ?? '';

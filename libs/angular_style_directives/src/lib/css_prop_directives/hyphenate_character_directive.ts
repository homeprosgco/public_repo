...
@Directive({
  selector: '[sgHyphenateCharacter]',
  standalone: true
})
export class HyphenateCharacterDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgHyphenateCharacter = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgHyphenateCharacter() ?? '';
      ...
      this.el.nativeElement.style.hyphenateCharacter = applied ?? '';

...
export class ScrollPaddingDirective {
  ...
  sgScrollPadding = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgScrollPadding() ?? '';
      ...
      this.el.nativeElement.style.scrollPadding = applied ?? '';

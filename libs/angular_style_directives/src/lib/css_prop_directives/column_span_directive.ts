...
@Directive({
  selector: '[sgColumnSpan]',
  standalone: true
})
export class ColumnSpanDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgColumnSpan = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgColumnSpan() ?? '';
      ...
      this.el.nativeElement.style.columnSpan = applied ?? '';

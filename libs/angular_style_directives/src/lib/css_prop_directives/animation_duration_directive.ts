...
@Directive({
  selector: '[sgAnimationDuration]',
  standalone: true
})
export class AnimationDurationDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgAnimationDuration = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgAnimationDuration() ?? '';
      ...
      this.el.nativeElement.style.animationDuration = applied ?? '';

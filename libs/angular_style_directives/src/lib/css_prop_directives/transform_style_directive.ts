...
@Directive({
  selector: '[sgTransformStyle]',
  standalone: true
})
export class TransformStyleDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgTransformStyle = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgTransformStyle() ?? '';
      ...
      this.el.nativeElement.style.transformStyle = applied ?? '';

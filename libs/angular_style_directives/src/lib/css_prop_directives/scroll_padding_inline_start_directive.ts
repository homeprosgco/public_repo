...
@Directive({
  selector: '[sgScrollPaddingInlineStart]',
  standalone: true
})
export class ScrollPaddingInlineStartDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgScrollPaddingInlineStart = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgScrollPaddingInlineStart() ?? '';
      ...
      this.el.nativeElement.style.scrollPaddingInlineStart = applied ?? '';

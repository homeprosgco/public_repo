...
@Directive({
  selector: '[sgIsolation]',
  standalone: true
})
export class IsolationDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgIsolation = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgIsolation() ?? '';
      ...
      this.el.nativeElement.style.isolation = applied ?? '';

...
@Directive({
  selector: '[sgMaskOrigin]',
  standalone: true
})
export class MaskOriginDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgMaskOrigin = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgMaskOrigin() ?? '';
      ...
      this.el.nativeElement.style.maskOrigin = applied ?? '';

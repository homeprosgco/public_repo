...
@Directive({
  selector: '[sgOverscrollBehaviorInline]',
  standalone: true
})
export class OverscrollBehaviorInlineDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgOverscrollBehaviorInline = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgOverscrollBehaviorInline() ?? '';
      ...
      this.el.nativeElement.style.overscrollBehaviorInline = applied ?? '';

...
@Directive({
  selector: '[sgListStylePosition]',
  standalone: true
})
export class ListStylePositionDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgListStylePosition = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgListStylePosition() ?? '';
      ...
      this.el.nativeElement.style.listStylePosition = applied ?? '';

...
@Directive({
  selector: '[sgBreakInside]',
  standalone: true
})
export class BreakInsideDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgBreakInside = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBreakInside() ?? '';
      ...
      this.el.nativeElement.style.breakInside = applied ?? '';

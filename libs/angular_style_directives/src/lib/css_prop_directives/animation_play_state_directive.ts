...
@Directive({
  selector: '[sgAnimationPlayState]',
  standalone: true
})
export class AnimationPlayStateDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgAnimationPlayState = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgAnimationPlayState() ?? '';
      ...
      this.el.nativeElement.style.animationPlayState = applied ?? '';

...
@Directive({
  selector: '[sgColumnRuleWidth]',
  standalone: true
})
export class ColumnRuleWidthDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgColumnRuleWidth = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgColumnRuleWidth() ?? '';
      ...
      this.el.nativeElement.style.columnRuleWidth = applied ?? '';

...
@Directive({
  selector: '[sgBorderInlineEndWidth]',
  standalone: true
})
export class BorderInlineEndWidthDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgBorderInlineEndWidth = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBorderInlineEndWidth() ?? '';
      ...
      this.el.nativeElement.style.borderInlineEndWidth = applied ?? '';

...
@Directive({
  selector: '[sgBorderInlineStartWidth]',
  standalone: true
})
export class BorderInlineStartWidthDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgBorderInlineStartWidth = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBorderInlineStartWidth() ?? '';
      ...
      this.el.nativeElement.style.borderInlineStartWidth = applied ?? '';

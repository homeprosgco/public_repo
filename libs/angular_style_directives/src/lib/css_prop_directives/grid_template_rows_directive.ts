...
@Directive({
  selector: '[sgGridTemplateRows]',
  standalone: true
})
export class GridTemplateRowsDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgGridTemplateRows = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgGridTemplateRows() ?? '';
      ...
      this.el.nativeElement.style.gridTemplateRows = applied ?? '';

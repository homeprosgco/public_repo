...
@Directive({
  selector: '[sgScrollPaddingTop]',
  standalone: true
})
export class ScrollPaddingTopDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgScrollPaddingTop = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgScrollPaddingTop() ?? '';
      ...
      this.el.nativeElement.style.scrollPaddingTop = applied ?? '';

...
@Directive({
  selector: '[sgBorderLeftStyle]',
  standalone: true
})
export class BorderLeftStyleDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgBorderLeftStyle = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBorderLeftStyle() ?? '';
      ...
      this.el.nativeElement.style.borderLeftStyle = applied ?? '';

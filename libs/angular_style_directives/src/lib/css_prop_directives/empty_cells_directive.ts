...
@Directive({
  selector: '[sgEmptyCells]',
  standalone: true
})
export class EmptyCellsDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgEmptyCells = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgEmptyCells() ?? '';
      ...
      this.el.nativeElement.style.emptyCells = applied ?? '';

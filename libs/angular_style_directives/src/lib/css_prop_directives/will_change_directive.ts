...
@Directive({
  selector: '[sgWillChange]',
  standalone: true
})
export class WillChangeDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgWillChange = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgWillChange() ?? '';
      ...
      this.el.nativeElement.style.willChange = applied ?? '';

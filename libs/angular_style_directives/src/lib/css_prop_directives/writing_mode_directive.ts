...
@Directive({
  selector: '[sgWritingMode]',
  standalone: true
})
export class WritingModeDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgWritingMode = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgWritingMode() ?? '';
      ...
      this.el.nativeElement.style.writingMode = applied ?? '';

...
@Directive({
  selector: '[sgBorderRight]',
  standalone: true
})
export class BorderRightDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgBorderRight = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBorderRight() ?? '';
      ...
      this.el.nativeElement.style.borderRight = applied ?? '';

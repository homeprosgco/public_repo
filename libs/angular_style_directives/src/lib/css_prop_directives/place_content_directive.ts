...
@Directive({
  selector: '[sgPlaceContent]',
  standalone: true
})
export class PlaceContentDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgPlaceContent = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgPlaceContent() ?? '';
      ...
      this.el.nativeElement.style.placeContent = applied ?? '';

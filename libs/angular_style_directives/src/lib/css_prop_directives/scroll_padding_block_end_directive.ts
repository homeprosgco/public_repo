...
@Directive({
  selector: '[sgScrollPaddingBlockEnd]',
  standalone: true
})
export class ScrollPaddingBlockEndDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgScrollPaddingBlockEnd = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgScrollPaddingBlockEnd() ?? '';
      ...
      this.el.nativeElement.style.scrollPaddingBlockEnd = applied ?? '';

...
@Directive({
  selector: '[sgBorderBlockEndColor]',
  standalone: true
})
export class BorderBlockEndColorDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgBorderBlockEndColor = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBorderBlockEndColor() ?? '';
      ...
      this.el.nativeElement.style.borderBlockEndColor = applied ?? '';

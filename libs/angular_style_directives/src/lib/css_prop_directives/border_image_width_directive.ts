...
@Directive({
  selector: '[sgBorderImageWidth]',
  standalone: true
})
export class BorderImageWidthDirective {
  private el = inject(ElementRef);
  private utils = inject(DirectiveUtils);
  private media = inject(MediaQueryService);

  sgBorderImageWidth = input<string | { [media: string]: string }>();

  constructor() {
    effect(() => {
      const raw = this.sgBorderImageWidth() ?? '';
      ...
      this.el.nativeElement.style.borderImageWidth = applied ?? '';

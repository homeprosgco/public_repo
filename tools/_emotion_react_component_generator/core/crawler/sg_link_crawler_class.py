...
# Patch SGLinkCrawler to use domain name as fallback template_name if <title> is missing

# Update SGLinkCrawler to include crawl timing
class SGLinkCrawler:
    def __init__(self, root_url: str):
        self.root_url = root_url
        self.visited = set()
        self.to_visit = [urldefrag(root_url)[0]]
        self.visting_url = ""
    ...
    def crawl(self, limit: int = 20):
        ...
        while self.to_visit and len(self.visited) < limit:
            ...
            if url == urldefrag(self.root_url)[0] and not self.template_name:
            ...
            self.stylesheets.update(urljoin(url, l.get("href")) for l in soup.find_all("link", rel="stylesheet") if l.get("href"))
            self.scripts.update(urljoin(url, s.get("src")) for s in soup.find_all("script") if s.get("src"))
            self.images.update(urljoin(url, i.get("src")) for i in soup.find_all("img") if i.get("src"))

            for tag in soup.find_all(style=True):
                matches = re.findall(r'url\([\'"]?(.*?)[\'"]?\)', tag["style"])
                self.images.update(urljoin(url, m) for m in matches)

            new_links = [urldefrag(urljoin(url, a.get("href")))[0] for a in soup.find_all("a") if a.get("href")]

            for link in new_links:
                if link not in self.visited and link.startswith(urldefrag(self.root_url)[0]):

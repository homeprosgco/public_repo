...
export class InvoiceResolver {
  ...
  #collectionPath(accountId: string, invoiceId: string) {
    return `${Tech1ProAccountsPath}/${accountId}/invoices/${invoiceId}`
  }
  ...
  async address(@Parent() invoice: Invoice, @Context() ctx: ServerContext) {
    const { addressId } = invoice;
    return this.addressSvc.getAddress(ctx.accountId, (addressId as string));
  }

  @ResolveField('activities')
  async activities(@Parent() invoice: Invoice, @Context() ctx: ServerContext) {
    const { id } = invoice;
    return this.activitySvc.getActivities(`${this.#collectionPath(ctx.accountId, (id as string))}`);
  }

  @ResolveField('comments')
  async comments(@Parent() invoice: Invoice, @Context() ctx: ServerContext) {
    const { id } = invoice;
    return this.commentSvc.getComments(`${this.#collectionPath(ctx.accountId, (id as string))}`);
  }

  @ResolveField('notes')
  async notes(@Parent() invoice: Invoice, @Context() ctx: ServerContext) {
    const { id } = invoice;

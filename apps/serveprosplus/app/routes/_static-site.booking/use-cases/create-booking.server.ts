...
function bookingValidator(fields: BookingFieldsType) {
  const { preferredBookingDates, bookingImageFolderId, ...rest } = fields;

  return Prisma.validator<Prisma.BookingCreateArgs>()({
    data: {
      ...rest,
...
export const createBooking = async (fields: BookingFieldsType) => {
  const result = BookingFields.safeParse(fields);

  if (!result.success) {
    return { fields, errors: result.error.flatten() }
  } else {
    const validBooking = bookingValidator(fields);
    const booking = await prisma.booking.create(validBooking);
    return { booking };
  }
}
